...
IsLinesFormCross( const TLinePoints& lp1, const TLinePoints& lp2 )
{
	...
	return l1.isPerpendicularTo( l2 ) && l1==l2 && ( lp1c - lp2c ).length() <= algo::kTol.equalPoint();
}
...
GetAllNeedData( AcDbObjectIdArray& aDimId, RCAD::SetPnts2d& setPnts, CListOfArc& aArcs, ListOfLinePoints& aLinePoints, ListOfExtPoints& aExtPoints )
{
	...
	typedef std::multimap< AcGePoint2d, AcDbObjectId, RCAD::LessPnt2d > TLinesCenter;
	TLinesCenter mapOfLinesCenter;
	
	typedef	ListOfLinePoints::iterator  LineIt;
	typedef std::multimap< AcGePoint2d, LineIt, RCAD::LessPnt2d > TLinesIndex;
	...
	for ( int dimCount=0; dimCount != n; ++dimCount )
	{
		...
		if ( pLine )
		{
			...
			AcGePoint2d cp = ( sp + ep.asVector() )/2;
			...
			{	
				...
				{
					TLinePoints	 lp2(*(*mapOfLinesIndex.lower_bound( cp )).second);	
				...
				//if ( mapOfLinesCenter.find( cp ) == mapOfLinesCenter.end() )
				
				mapOfLinesCenter.insert( pair< AcGePoint2d, AcDbObjectId >( cp, DimId ) );
				mapOfLinesIndex.insert( pair< AcGePoint2d, LineIt >( cp, aLinePoints.end()-1 ) );
...
IsDimPointSnapToLine( const AcGeVector2d& vecOfMeasur, const AcGePoint2d& point, const ListOfLinePoints& aLinePoints )
{
	...
	{
		if ( IsPointsProjectToOnePointOnMeasureLine( vecOfMeasur, point, it_l->first ) &&
			 IsPointsProjectToOnePointOnMeasureLine( vecOfMeasur, point, it_l->second ) )
		{
